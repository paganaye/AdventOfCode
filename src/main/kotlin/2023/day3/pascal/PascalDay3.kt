val day3PascalSampleDataPart1 = """467..114..
...*......
..35..633.
......#...
617*......
...
fun day3() {
    ...
    data class Number(val x: Int, val y: Int, var value: Int, var length: Int = 1)


    fun symbolIsTouchingNumber(s: Symbol, n: Number): Boolean {
        return (s.y >= n.y - 1 && s.y <= n.y + 1
                && s.x >= n.x - 1 && s.x <= n.x + n.length)
    }

    fun parseSymbolsAndNumbers(input: String): Pair<MutableList<Symbol>, MutableList<Number>> {
        val lines = input.split("\n")
        val symbols = mutableListOf<Symbol>()
        val numbers = mutableListOf<Number>()
        var lastNumber = Number(Int.MIN_VALUE, Int.MIN_VALUE, 0, 0)
        lines.forEachIndexed({ y, line ->
            line.forEachIndexed({ x, ch ->
                if (ch.isDigit()) {
                    if (y == lastNumber.y && x == lastNumber.x + lastNumber.length) {
                        lastNumber.length += 1
                        lastNumber.value = lastNumber.value * 10 + (ch - '0')
                    } else {
                        lastNumber = Number(x, y, ch - '0')
                        numbers.add(lastNumber)
                    }
                } else if (ch != '.') {
                    symbols.add(Symbol(x, y, ch))
                }
            })
        })
    ...
    fun day3Part1(input: String): Int {
        val (symbols, numbers) = parseSymbolsAndNumbers(input)
        val partNumbers =
            numbers.filter { number -> symbols.any { symbol -> symbolIsTouchingNumber(symbol, number) } }
    ...
    fun day3Part2(input: String): Int {
        ...
        val gearsValues =
            symbols.map { symbol ->
                val numbersAroundSymbol = numbers.filter { number -> symbolIsTouchingNumber(symbol, number) }
                if (numbersAroundSymbol.count() == 2) {
                    numbersAroundSymbol[0].value * numbersAroundSymbol[1].value
                } else 0
            }
        val result = gearsValues.sum()
        return result
    }


    println("  * Part 1")
    println("    sample data: ${day3Part1(day3PascalSampleDataPart1)}")
    println("    main data: ${day3Part1(day3PascalMainData)}")
    println("  * Part 2")
    println("    sample data: ${day3Part2(day3PascalSampleDataPart1)}")
    println("    main data: ${day3Part2(day3PascalMainData)}")
}
