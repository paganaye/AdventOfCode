val day1PascalSampleDataPart1 = """1abc2
pqr3stu8vwx
a1b2c3d4e5f
treb7uchet"""
val day1PascalSampleDataPart2 = """two1nine
eightwothree
abcone2threexyz
xtwone3four
4nineeightseven2
zoneight234
7pqrstsixteen"""
val day1PascalMainData = """ckmb52fldxkseven3fkjgcbzmnr7
...
fun day1_part1() {
    println("  Part 1")
    fun sumCalibrationValues(text: String): Int {
    ...
    println("    sample data: ${sumCalibrationValues(day1PascalSampleDataPart1)}")
    println("    main data: ${sumCalibrationValues(day1PascalMainData)}")
}

fun day1_part2() {
    println("  Part 2")
    fun sumCalibrationValuesTwo(text: String): Int {
        val lines = text.split("\n")
        val digits = mapOf(
            "0" to 0,
            "1" to 1,
        ...
        data class ValueAndPos(val value: Int, val position: Int)

        val values = lines.mapNotNull { line ->
            var bestFirst: ValueAndPos? = null
            var bestLast: ValueAndPos? = null
            for (entry in digits) {
                val firstIndex = line.indexOf(entry.key)
                if (firstIndex >= 0) {
                    if (bestFirst == null || firstIndex < bestFirst.position) bestFirst =
                        ValueAndPos(entry.value, firstIndex)
                    val lastIndex = line.lastIndexOf(entry.key)
                    if (bestLast == null || lastIndex > bestLast.position) bestLast =
                        ValueAndPos(entry.value, lastIndex)
                }
            }
            if (bestFirst != null && bestLast != null) {
                (bestFirst.value * 10 + bestLast.value)
            } else null
        }
        return values.sum()
    }

    println("    sample data: ${sumCalibrationValuesTwo(day1PascalSampleDataPart2)}")
    println("    main data: ${sumCalibrationValuesTwo(day1PascalMainData)}")
}
